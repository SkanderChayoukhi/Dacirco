...
class TCWorker:
    ...
    def __init__(
        ...
        self.file_download_task_queue = asyncio.Queue[GrpcTCTask]()
        self.ffmpeg_task_queue = asyncio.Queue[GrpcTCTask]()
        self.file_upload_task_queue = asyncio.Queue[GrpcTCTask]()
    ...
    async def register_worker(self) -> None:
        if self.stub:
            response: GrpcServiceReply = await self.stub.register_worker(
                GrpcWorkerDesc(name=self.name, id=self.id)
            )
    ...
    async def run_ffmpeg(self) -> None:
        while True:
            ...
            if self.stub:
                ...
                _logger.debug("ffmpeg command: %s", " ".join(args))
